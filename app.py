...
# Filter DataFrame safely (handling partial date range selection)
if len(date_range) == 2:
    ...
    st.warning("Please select both a start and end date in the sidebar calendar.")
    st.stop()

# Prevent crashes when no data matches selected criteria
if filtered_df.empty:
    st.warning("⚠️ No data available for the selected filters. Please expand your selection criteria.")
    st.stop()
...
with col2:
    ...
    if not profit_over_time.empty and profit_over_time['Profit'].notna().any():
...
selected_category = st.selectbox("Select a Category to Explore Sub-Categories", available_categories)
